...
describe('Evidence pipeline', () => {
  it('normalizes and persists signals and observations as immutable Evidence', async () => {
    ...
    const result = await new EvidencePipeline(runtime, new (require('../../../src/providers/adapters/MockProvider').MockProvider)()).executeFlow(artifact(), [{ metric: 'views', value: 42, observedAt: 1000 }], [{ event: 'Human reviewer confirmed publication', observedAt: 1000 }]);
